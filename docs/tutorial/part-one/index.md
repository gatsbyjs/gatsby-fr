...
Le component Gatsby `<Link />` permet de lier différentes pages de votre site web. Pour les liens externes non géré par votre site web Gatsby, utilisez la balise HTML par défaut `<a>`.

## Déployer un site web Gatsby

Gatsby.js est un _générateur de site web moderne_, ce qui veut dire qu'il n'y a pas besoin de serveurs à configurer ou de base de données à déployer. Au lieu de ça, la commande Gatsby `build` produit un dossier contenant des fichiers HTML statiques ainsi que des fichiers JavaScript que vous pouvez déployer sur n'importe quel hébergeur de site web statique.

Essayez d'utiliser [Surge](http://surge.sh/) pour déployer votre premier site web Gatsby. Surge est un des nombreux "hébergeurs de site web statique" ce qui lui permet de déployer des site web Gatsby.
...
Ensuite, construisez votre site web en lancant ces commanes dans votre terminal à la racine de votre site web (conseil: vérifiez que vous êtes à la racine de votre site web, dans ce cas dans le dossier hello-world, ce que vous pouvez faire en un nouvel onglet dans la même fenêtre où vous aviez lancé `gatsby develop`):

```shell
gatsby build
```

La construction devrait prendre 15-30 secondes. Dès que la construction est finie, il est intéressant d'aller voir aux fichiers que la commande `gatsby build` nous a préparé pour le déploiement.

Jetez un coup d'oeil aux fichiers générés en tapant cette commande à la racine de votre site web, ce qui vous permettra de voir le dossier `public` :

```shell
ls public
```

Finalement, déployez votre site web en publiant les fichiers générés sur surge.sh.
...
![Capture d'écran d'une publication de site web Gatsby en utilisant Surge](surge-deployment.png)

Ouvrez l'adresse donnée sur la ligne du bas (`lowly-pain.surge.sh` dans ce cas) et vous allez y voir votre nouveau site web ! Bon travail !
...
- Avez appris sur les components
- Avez appris sur les composants et sous-composants de pages Gatsby
- Avez appris sur les “props” React et la réutilisation de components React

Maintenant, continuez avec [**ajouter du styles à votre site web**](/tutorial/part-two/)!
